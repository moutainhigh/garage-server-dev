  ...
  <div class="info-bg">
    <div class="bg-class">
      ...
      <div class="textCenter marginB6" style="padding-bottom:20px;">   
         <Button size="large" @click="applyBtn" type="primary" v-if="$route.query.type=='edit'">处理</Button>      
        <Button size="large" @click="approval" type="primary" v-if="$route.query.type=='approve'">审批通过</Button>
        <Button size="large" @click="refuse" type="primary" v-if="$route.query.type=='approve'&&createFinaForm.pushSourceStr != '贷后推送'">审批驳回</Button>
        <Button size="large" @click="$router.push('/sgOutTasks_list')">取消</Button>
      </div>
  ...
  methods: {
    ...
    approval() {//审批通过
      let list = {
        ...
        pushSource:this.createFinaForm.pushSource
      };
      this.$http
        .post("/garage/sgGarageOrder/approveGarageOut", list)
        .then(function(res) {
          if (res.data.success == true) {
             this.$Modal.success({
              ...
              onOk: () => {
                 this.$router.push("/sgOutTasks_List");
              }
            });
          } else {
            this.$Modal.warning({
    ...
    refuse() {//审批拒绝
      ...
      this.$http.post("/garage/sgGarageOrder/reject", list).then(function(res) {
        if (res.data.success == true) {
           this.$Modal.success({
              title: "提示",
              content: res.data.data,
    ...
    handle(){
       let that = this;
          ...
          if(this.createFinaForm.actualStartTime){
            list.actualStartTime=this.dateToString(this.createFinaForm.actualStartTime);  
          }
          if(this.createFinaForm.predictEndTime){
            list.predictEndTime=this.dateToString(this.createFinaForm.predictEndTime);  
          }
    ...
    getDetails() {
      ...
      let list = {
        sgGarageDetailId: this.$route.query.id
      };
      this.$http
        .post("/garage/sgGarageOrder/getBill?sgGarageDetailId=" + id)
        .then(function(res) {
          if (res.data.success == true) {
            ...
            this.createFinaForm.actualStartTime = res.data.data.actualStartTime;            
            this.photoData = res.data.data.attachSourceDTOList;
            for(let n in this.photoData){
              this.attIds.push(this.photoData[n].id);
            }
