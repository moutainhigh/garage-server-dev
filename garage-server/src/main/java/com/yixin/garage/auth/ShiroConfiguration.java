...
import com.yixin.garage.auth_local.ShiroConfigurationLocal;
...
@Configuration
@ConditionalOnMissingBean(value=ShiroConfigurationLocal.class)
@Slf4j
...
/*    @Bean
    ...
    public CasFilter getCasFilter() {
        ...
        casFilter.setFailureUrl(casServerUrlPrefix + "/login?service=" + casService);// 我们选择认证失败后再打开登录页面
        return casFilter;
    }
    ...
    public ShiroFilterFactoryBean getShiroFilterFactoryBean(DefaultWebSecurityManager securityManager, CasFilter casFilter) {
        ...
        shiroFilterFactoryBean.setLoginUrl(casServerUrlPrefix + "/login?service=" + casService);
        // 登录成功后要跳转的连接
