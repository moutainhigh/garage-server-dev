...
class TemplatedExcelColumn {
	...
	TemplatedExcelColumn(int index, String fieldName, ValueFormatter valueFormatter) {
		super();
		if(index < 0){
			throw new java.lang.IllegalArgumentException("类索引必须为>=0的整数，当前索引值为:"+index);
		}
		if(fieldName==null || "".equals(fieldName.trim())){
			throw new java.lang.IllegalArgumentException("字段名不能为空");
		}
		if(valueFormatter == null){
			throw new java.lang.IllegalArgumentException("格式化器valueFormater不能为null");
		}
