...
public class MyShiroCasRealm extends AuthorizingRealm{
	...
	private final static Logger logger = LoggerFactory.getLogger(MyShiroCasRealm.class);
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token)
        ...
        AuthenticationInfo authInfo = new SimpleAuthenticationInfo(token.getPrincipal()
                , token.getCredentials(), this.getName());
        //AuthenticationInfo doGetAuthenticationInfo = token;
        ...
        if (session.getAttribute("authenticationInfo_shiro") == null
            ...
            UserDto userDto = getUserDto(username);
            if(userDto==null){
            	logger.error("用户{}不存在,请检查！",username);
            	throw new BzException("用户不存在！");
            }
            session.setAttribute("authenticationInfo_shiro", authInfo);
            ...
            if (session.getAttribute("userInfo_shiro") == null) {
                authorizationInfo = userDto.getSimpleAuthorizationInfo();
                session.setAttribute("userInfo_shiro", authorizationInfo);
                logger.info("将用户角色信息放入到session中：{}",JSONObject.toJSONString(authorizationInfo));
            }
        } else {
            authInfo = (AuthenticationInfo) session
